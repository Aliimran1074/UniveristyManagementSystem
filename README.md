# 🤖 SaaS Learning Management System with Agentic AI

A modern **AI-powered Learning Management System (LMS)** built to help educational institutes manage students, teachers, courses, subscriptions, and AI-powered learning automation.

The system provides multiple user roles with dedicated dashboards and integrates Agentic AI features to automate academic tasks like assignment generation, quiz creation, and evaluation.

---

# 🚀 Features

## 👨‍💼 Super Admin

- Manage multiple institutes
- Monitor institute activities
- Manage subscriptions
- View AI usage analytics
- Dashboard statistics


## Screenshots

### Super Admin Dashboard

![Dashboard](./screenshots/super%20admin%201.png)
![Dashboard](./screenshots/super%20admin%202.png)
![Dashboard](./screenshots/super%20admin%203.png)
![Dashboard](./screenshots/super%20admin%204.png)
![Dashboard](./screenshots/super%20admin%205.png)
![Dashboard](./screenshots/super%20admin%20auto%20report.png)

---


## 🏫 Institute Management

- Manage institutes
- Manage institute information
- Track subscription plans
- Monitor AI feature usage
- Manage Students and Faculty of particular institute

## Screenshots

### Institute Admin Dashboard
![Dashboard](./screenshots/institute%20admin%201.png)
![Dashboard](./screenshots/institute%20admin%202.png)
![Dashboard](./screenshots/institute%20admin%203.png)
![Dashboard](./screenshots/institute%20admin%204.png)-

---

## 👨‍🏫 Teacher Features

- AI Agent Control Panel
- Generate assignments using AI
- Generate quizzes using AI
- AI-based assignment checking
- AI-based quiz checking


## Screenshots

### Teacher Admin Dashboard
## Teacher Dashboard

![Teacher Dashboard 1](./screenshots/teacher%20dashboard%201.png)
![Teacher Dashboard 2](./screenshots/teacher%20dashboard%202.png)
![Teacher Dashboard 3](./screenshots/teacher%20dashboard%203.png)
![Teacher Dashboard 4](./screenshots/teacher%20dashboard%204.png)
![Teacher Dashboard 5](./screenshots/teacher%20dashboard%205%2B.png)
![Teacher Dashboard 6](./screenshots/teacher%20dashboard%206.png)
![Teacher Dashboard 7](./screenshots/teacher%20dashboard%207.png)
![Teacher Dashboard 8](./screenshots/teacher%20dashboard%208.png)
![Teacher Dashboard 9](./screenshots/teacher%20dashboard%209.png)
![Teacher Dashboard 10](./screenshots/teacher%20dashboard%2010.png)
![Teacher Dashboard 11](./screenshots/teacher%20dashboard%2011.png)
---

## 👨‍🎓 Student Features

- View enrolled courses
- Access assignments
- Submit academic work
- Track learning progress

## Screenshots
### Student Admin Dashboard  
![Dashboard](./screenshots/student%20dashboard%201.png)
![Dashboard](./screenshots/student%20dashboard%202.png)
![Dashboard](./screenshots/student%20dashboard%203.png)
![Dashboard](./screenshots/student%20dashboard%204.png)
![Dashboard](./screenshots/student%20dashboard%205.png)
![Dashboard](./screenshots/student%20dashboard%206.png)
---

# 🧠 Agentic AI Features

The system includes AI agents designed for educational automation.

### ✨ Assignment Generator

Automatically generates assignments from given topics using AI.

Features:

- Topic based generation
- Difficulty selection
- Question count control
- PDF generation


---

### ✨ Quiz Generator

Creates AI-powered quizzes.

Features:

- MCQ generation
- Difficulty control
- Topic based quizzes


---

### ✨ AI Assignment Checker

Automatically evaluates submitted assignments.


---

### ✨ AI Quiz Checker

Checks quiz responses and provides evaluation.


---

# 🛠️ Tech Stack


## Frontend

- React.js
- Tailwind CSS
- ShadCN UI
- Axios
- Lucide React


## Backend

- Node.js
- Express.js
- MongoDB
- Mongoose


## AI Integration

- Large Language Models
- Prompt Engineering
- AI Agents
- Generative AI


## Other Tools

- ImageKit (File Storage)
- PDF Generation


---

# 📂 Project Structure

